...
function createClient(url, endpoint, callback) {
...
function createServer(options, callback) {
  ...
  if (xml) {
    var wsdl = new WSDL(xml || services, options.uri, options);
    var ss = new Server(null, path, services, wsdl);
    ss.log = console.log;
    callback(null, ss);
  }
  else {
    console.log("Async server build");
    _requestWSDL(options.uri, options, function(err, wsdl) {
      if (err) {
        console.log(err);
        //server.close();
        callback(err);
...
function listen(server, pathOrOptions, services, xml) {
    ...
    if (xml) {
      var wsdl = new WSDL(xml || services, options.uri, options);
      var ss = new Server(server, path, services, wsdl);
      ss.log = console.log;
      return ss;
    }
    else {
