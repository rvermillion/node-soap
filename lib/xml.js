...
XMLNS.prototype.add = function(pfx, uri) {
  if (pfx && uri) {
    ...
    console.log("Could not add(%s, %s)", pfx, uri);
  }
};
...
ParsedObject.prototype.getTypeElementMap = function(type, map, ns, seq) {
  var parsed = this;
  if (!map) map = {};
  if (type && type.children) {
    type.children.forEach(function(child) {
      if (child.name == "element") {
        var cn = child.$name;
        if (seq) seq.push(cn);
        console.log("Adding element:", cn, "in NS:", ns);
        map[cn] = child;
        if (!child.$targetNamespace) child.$targetNamespace = ns;
      }
      else if (child.name == "sequence") {
        parsed.getTypeElementMap(child, map, ns, seq);
      }
      else if (child.name == "choice") {
        parsed.getTypeElementMap(child, map, ns);
      }
    });
  }
  return map;
...
ParsedObject.prototype.appendObjectToXML = function(parts, obj, path, element, nsPrefix, nsUri, xmlns) {
  ...
  if (schema) {
    if (typeNs != schema.$targetNamespace) {
      console.log("Found a weird Namespace problem:", typeNs, "!=", schema.$targetNamespace);
    }


    type = schema.complexTypes[typeName];
    if (type) {
      console.log("Found complex type:", typeName, "in schema:", schema.$targetNamespace);
    }
    else {
      ...
      if (type) {
        ...
        console.log("Could not find type:", typeName, "for element", element, "in schema", schema.$targetNamespace);
      }
    }
  }
  else {
  ...
  if (typeRef) {
    self.getTypeElementMap(type, elementMap, typeNs, elementSeq);
    //console.log("Element Map:", elementMap);
  }

  var xmlnsAttrib = '', pfx, ns;

  pfx = xmlns.getPrefix(nsUri /*typeNs*/, elementSchema.targetNsPrefix||elementSchema.$targetNSPrefix||"tns") || nsPrefix;
  ns = pfx ? pfx + ':' : '';
  ...
  if (Array.isArray(obj)) {
    ...
    function writeAttr(name) {
      var child = obj[name];
      var childElement = elementMap[name];
      if (!childElement) {
        console.log("Could not find child '%s'", name, "in elementMap", elementMap, "for element", element);
        childElement = {};
      }
      path.push(name);
    ...
    if (elementSeq.length) {
      console.log("Writing in sequence order:", elementSeq);
      elementSeq.forEach(function(name) {
        if (name in obj) {
          console.log("Writing sequence child:", name);
          writeAttr(name);
        }
      });
    }
    else {
      Object.keys(obj).forEach(writeAttr);
    }
